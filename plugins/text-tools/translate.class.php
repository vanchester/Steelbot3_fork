    ...
    public function translateText($text, $fromLanguage = "en", $toLanguage = "ru", $translit = false) 
    {
        if (empty($this->_errors)) {
            ...
            for($i = 0; $i < strlen($text); $i += 1000)
            {
				...
				$response = $this->_curlToGoogle("http://translate.google.com/translate_a/t?client=te&text=".urlencode($subText)."&hl=ru&sl={$fromLanguage}&tl={$toLanguage}&multires=1&otf=1&ssel=0&tsel=0&uptl=ru&sc=1");
