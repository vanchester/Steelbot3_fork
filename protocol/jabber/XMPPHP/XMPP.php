...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	public function message_handler($xml) {
        ...
        /**
		 * checking for invite (added by nexor) =================
		 */

		$inv = $xml->sub('x', null,'http://jabber.org/protocol/muc#user');
		if ($inv && $inv->sub('invite')) {
		  $payload['subtype'] = 'invite';
		}
		/** ======================================================= */

        /**
		 * checking for delayed message (added by nexor) =================
		 */
        $payload['subtype'] = 'normal';
        if ( $xml->hasSub('x', 'jabber:x:delay') ) {
            $payload['subtype'] = 'delayed';
        }
        /** ====================================================== */
